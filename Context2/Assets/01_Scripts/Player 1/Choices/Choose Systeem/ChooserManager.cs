...
public class ChooserManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        playernetwork = client.GetClientPlayerNetwork();
        //Debug.Log(" player we got : " + playernetwork.OwnerClientId);
    }
    ...
    public void NextChoice(ChoiceObject.ChoiceImpact _madeChoice)
    {
        ...
        if (choiceCounter < AllChoices.Length - 1)
        {
            ...
            choiceCounter++;
            //lDebug.Log(AllChoices[choiceCounter].name);
        }
        else
        {
    ...
    private void PlayChoiceConsequenceEvent(ChoiceObject.ChoiceImpact _madeChoice)
    {
        if (_madeChoice == ChoiceObject.ChoiceImpact.Yes)
        {
            ...
            if (choiceEvent != null)
            {
                ...
                int index = eventListScripableObject.gameEvents.IndexOf(choiceEvent);
                playernetwork.PrepareEventpackage(index);
            }
        }
        else
        {
